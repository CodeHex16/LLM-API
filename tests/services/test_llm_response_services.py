...
def test_llm_response_service_get_context(monkeypatch):
    ...
    monkeypatch.setattr(
        llm_response_service._vector_database, "search_context", lambda q: mock_context
    )
...
@pytest.mark.xfail(
    reason="This test is expected to fail due to unimplemented functionality"
)
def test_llm_response_service_generate_response(monkeypatch):
    ...
    monkeypatch.setattr(
        llm_response_service._LLM._model,
        "stream",
        lambda messages: mock_stream_response,
    )
