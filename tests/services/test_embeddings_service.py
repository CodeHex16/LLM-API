...
def test_openai_embedding_provider(monkeypatch):
    embedding_provider = OpenAIEmbeddingProvider(
        "test_api_key", "text-embedding-ada-002"
    )
    assert isinstance(
        embedding_provider, OpenAIEmbeddingProvider
    ), "Should return an instance of OpenAIEmbeddingProvider"
    assert (
        embedding_provider._api_key == "test_api_key"
    ), "Should use the mocked API key"
    assert (
        embedding_provider._model_name == "text-embedding-ada-002"
    ), "Should use the mocked model name"
...
def test_get_embeddings_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.embeddings_service.settings.LLM_PROVIDER", "openai"
    )

    embedding_provider = get_embedding_provider()
    assert isinstance(
        embedding_provider, OpenAIEmbeddingProvider
    ), "Should return an instance of EmbeddingsService"


def test_get_embedding_provider_invalid(monkeypatch):
    # Mock the environment variable for an invalid provider
    monkeypatch.setattr(
        "app.services.embeddings_service.settings.LLM_PROVIDER", "invalid_provider"
    )
